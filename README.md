...
Multi-child parent rows have a disclosure control. Collapsing a parent hides its child rows without changing task data, completion state, local `##N` selectors, or manual order; expanding it restores the same visible hierarchy. Collapse choices are remembered across normal app launches. A one-child project stays in its existing compact form and does not show a redundant disclosure control. Adding a child to a collapsed parent expands it automatically and opens the new child.
